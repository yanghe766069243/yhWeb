...
namespace NewSchoolLive.RabbitMQ
{
    ...
    public class RabbitMQInvoker
    {
        ...
        public RabbitMQInvoker(IOptionsMonitor<RabbitMQOptions> optionsMonitor)
        { 
        ...
        private static ConcurrentDictionary<string, bool> RabbitMQInvoker_ExchangeQueue = new ConcurrentDictionary<string, bool>();
                ...
                lock (RabbitMQInvoker_InitLock)
                {
                    if (_CurrentConnection == null || !_CurrentConnection.IsOpen)
                    {
                        var factory = new ConnectionFactory()
                        {
                            ...
                            VirtualHost=this._rabbitMQOptions.VirtualHost,
                            AutomaticRecoveryEnabled=true,//true,//如果connection挂掉是否重新连接
                            TopologyRecoveryEnabled= true//连接恢复后，连接的交换机，队列等是否一同恢复
                        };
                        _CurrentConnection = factory.CreateConnection(new string[] {
                        this._rabbitMQOptions.HostName
                        });
                    }
                }
            }
        ...
        public void SendDelay(string targetExchangeName, string message, int delaySecond)
        {
            ...
            using (var channel = _CurrentConnection.CreateModel())//开辟新的信道通信
            {
                try
                {
                    string delayExchangeName = "MSA_DelayExchange";
                    ...
                    channel.QueueDeclare("MSA_DelayQueue", true, false, false, args);
                    channel.QueueBind(queue: "MSA_DelayQueue",
